...
async function loadData() {
  ...
  return (async () => {
    ...
    if (src.length === 0) {
      ...
      function mapKategori(googleCategory) {
        ...
        if (cat.includes("biography") || cat.includes("memoir"))
          return "biografi";
        if (cat.includes("comic") || cat.includes("graphic novel"))
          return "komik";
        ...
        if (cat.includes("language") || cat.includes("foreign"))
          return "bahasa";
        if (cat.includes("photography") || cat.includes("art"))
          return "fotografi";
        if (cat.includes("reference")) return "referensi";
        if (cat.includes("business") || cat.includes("economics"))
          return "nonfiksi";
    ...
    console.log("Kategori tersimpan:", [
      ...new Set(allBooks.map((b) => b.kategori)),
    ]);
...
(function setupDetailModal() {
  ...
  function getIdxFromBtn() {
    const idx =
      buyNowBtn && buyNowBtn.dataset && buyNowBtn.dataset.idx
        ? parseInt(buyNowBtn.dataset.idx, 10)
        : null;
  ...
  function updateSubtotalForIdx(idx) {
    ...
    const qty = Math.max(
      1,
      Math.min(parseInt(qtyInput.value || 1, 10), book.stock || 0),
    );
    qtyInput.value = qty;
  ...
  if (addToCartBtn)
    addToCartBtn.addEventListener("click", () => {
      const idx = addToCartBtn.dataset.idx
        ? parseInt(addToCartBtn.dataset.idx, 10)
        : null;
  ...
  if (buyNowBtn)
    buyNowBtn.addEventListener("click", () => {
      const idx = buyNowBtn.dataset.idx
        ? parseInt(buyNowBtn.dataset.idx, 10)
        : null;
      if (idx === null) return alert("Tidak ada buku yang dipilih.");
      const book = allBooks[idx];
...
function initRekomCarousel() {
  ...
  function update() {
    ...
    const translate =
      centerOffset - rekState.current * (rekState.slideWidth + rekState.gap);
    track.style.transform = `translateX(${translate}px)`;

    // Tambahkan class 'is-center' ke slide yang sedang aktif
    slides.forEach((s, i) =>
      s.classList.toggle("is-center", i === rekState.current),
    );
  ...
  function showKeyboardHint() {
    ...
    showKeyboardHint._t = window.setTimeout(
  ...
  if (rekState._keydownHandler)
    document.removeEventListener("keydown", rekState._keydownHandler);
  ...
  function startAutoplay() {
    ...
    autoTimer = window.setInterval(() => {
      ...
      if (rekState.current >= rekState.slides - 1) rekState.current = 0;
      else rekState.current += 1;
      update();
    }, autoplayDelay);
  }
  ...
  if (wrapperEl) {
    ...
    if (rekState._wheelHandler)
      wrapperEl.removeEventListener("wheel", rekState._wheelHandler, {
        passive: false,
      });
    ...
    wrapperEl.addEventListener("wheel", rekState._wheelHandler, {
      passive: false,
    });
  }

  // Pause autoplay saat tab/halaman tidak terlihat (user pindah tab)
  if (rekState._visibilityHandler)
    document.removeEventListener(
      "visibilitychange",
      rekState._visibilityHandler,
    );
  rekState._visibilityHandler = () => {
  ...
  if (rekState._resizeHandler)
    window.removeEventListener("resize", rekState._resizeHandler);
    ...
    .forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        const topBtn = document.querySelector(
          `.kategori-filter button[data-filter='${category}']`,
        );
